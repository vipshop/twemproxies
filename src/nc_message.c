...
struct msg *
msg_tmo_min(struct msg_base *msgb)
{
    struct rbnode *node;

    node = rbtree_min(&msgb->tmo_rbt);
    if (node == NULL) {
        return NULL;
    }
...
msg_tmo_delete(struct msg *msg)
{
    ...
    rbtree_delete(&mb->tmo_rbt, node);

    log_debug(LOG_VERB, "delete msg %"PRIu64" from tmo rbt", msg->id);
}

static struct msg *
_msg_get(struct msg_base *msgb)
{
    struct msg *msg;

    if (!TAILQ_EMPTY(&msgb->free_msgq)) {
        ASSERT(msgb->nfree_msgq > 0);

        msg = TAILQ_FIRST(&msgb->free_msgq);
        msgb->nfree_msgq--;
        TAILQ_REMOVE(&msgb->free_msgq, msg, m_tqe);
        goto done;
    }
...
struct msg *
msg_get(struct conn *conn, bool request, unsigned source_type, struct msg_base *msgb)
{
    struct msg *msg;
    ...
    if (source_type == NC_SOURCE_TYPE_REDIS) {
        msg->source_type = NC_SOURCE_TYPE_REDIS;
        if (request) {
        ...
        msg->source_type = NC_SOURCE_TYPE_MC;
        if (request) {
        ...
        msg->source_type = NC_SOURCE_TYPE_PROXY;
        if (request) {
            msg->parser = manage_parse_req;
            msg->noforward = 1;
        } else {
            msg->parser = NULL;
        }
        msg->add_auth = manage_add_auth;
        msg->fragment = manage_fragment;
        msg->reply = manage_reply;
        msg->failure = manage_failure;
        msg->pre_coalesce = manage_pre_coalesce;
        msg->post_coalesce = manage_post_coalesce;
    } else {
        NOT_REACHED();
    }
...
struct msg *
msg_get_error(unsigned source_type, err_t err, struct msg_base *msgb)
{
    struct msg *msg;
    struct mbuf *mbuf;
    int n;
    char *errstr = err ? strerror(err) : "unknown";
    char *protstr = source_type==NC_SOURCE_TYPE_REDIS ? "-ERR" : "SERVER_ERROR";
...
msg_put(struct msg *msg)
{
    ...
    msgb->nfree_msgq++;
    TAILQ_INSERT_HEAD(&msgb->free_msgq, msg, m_tqe);
}
...
void
msg_init(struct msg_base *msgb, struct mbuf_base *mbufb)
{
    log_debug(LOG_DEBUG, "msg size %d", sizeof(struct msg));
    msgb->msg_id = 0;
    msgb->frag_id = 0;
    msgb->nfree_msgq = 0;
    TAILQ_INIT(&msgb->free_msgq);
    rbtree_init(&msgb->tmo_rbt, &msgb->tmo_rbs);
    msgb->mbufb = mbufb;
}

void
msg_deinit(struct msg_base *msgb)
{
    struct msg *msg, *nmsg;

    for (msg = TAILQ_FIRST(&msgb->free_msgq); msg != NULL;
         msg = nmsg, msgb->nfree_msgq--) {
        ASSERT(msgb->nfree_msgq > 0);
...
{
    struct msg_base *msgb = msg->mb;
    struct mbuf_base *mbufb = msgb->mbufb;
    struct mbuf *mbuf;
...
msg_append(struct msg *msg, uint8_t *pos, size_t n)
{
    ...
    ASSERT(n <= mbuf_data_size(msg->mb->mbufb));
...
/*
 * Append n bytes of data into mbuf
 */
rstatus_t
msg_append_full(struct msg *msg, uint8_t *pos, size_t n)
{
    ...
    while (left > 0) {
        if (STAILQ_EMPTY(&msg->mhdr) ||
            mbuf_size(STAILQ_LAST(&msg->mhdr, mbuf, next)) == 0) {
            mbuf = mbuf_get(mbufb);
            if (mbuf == NULL) {
                return NC_ENOMEM;
            }
...
inline uint64_t
msg_gen_frag_id(struct msg_base *msgb)
{
    return ++msgb->frag_id;
}
...
msg_parsed(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    nmsg = msg_get(msg->owner, msg->request, conn->source_type, msgb);
    if (nmsg == NULL) {
        mbuf_put(nbuf);
        return NC_ENOMEM;
    }
...
int
nc_memory_init(struct context *ctx, size_t mbuf_chunk_size)
{
    /* init the mbuf_base  */
    ctx->mbufb = nc_alloc(sizeof(struct mbuf_base));
    if (ctx->mbufb == NULL) {
        return NC_ENOMEM;
    }

    mbuf_init(mbuf_chunk_size, ctx->mbufb);

    /* init the mbuf_base  */
    ctx->msgb = nc_alloc(sizeof(struct msg_base));
    if (ctx->msgb == NULL) {
        return NC_ENOMEM;
    }

    msg_init(ctx->msgb, ctx->mbufb);

    return NC_OK;
}

void
nc_memory_deinit(struct context *ctx)
{
    /* deinit the mbuf_base  */
    if (ctx->mbufb != NULL) {
        mbuf_deinit(ctx->mbufb);
        nc_free(ctx->mbufb);
        ctx->mbufb = NULL;
    }
