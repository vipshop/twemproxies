                    ...
                    uint32_t key_step)
{
    ...
    r->frag_id = msg_gen_frag_id(r->mb);
    r->nfrag = 0;
    r->frag_owner = r;
    ...
    for (i = 0; i < array_n(r->keys); i++) {        /* for each key */
        ...
        if (sub_msgs[idx] == NULL) {
            sub_msgs[idx] = msg_get(r->owner, r->request, r->source_type, r->mb);
            if (sub_msgs[idx] == NULL) {
                nc_free(sub_msgs);
                return NC_ENOMEM;
            }
...
redis_add_auth(struct context *ctx, struct conn *c_conn, struct conn *s_conn)
{
    ...
    msg = msg_get(c_conn, true, c_conn->source_type, ctx->msgb);
    if (msg == NULL) {
        c_conn->err = errno;
        return NC_ENOMEM;
    }
...
redis_post_connect(struct context *ctx, struct conn *conn, struct server *server)
{
    ...
    ASSERT(conn->source_type == NC_SOURCE_TYPE_REDIS);
     ...
     */
    msg = msg_get(conn, true, conn->source_type, ctx->msgb);
    if (msg == NULL) {
        return;
    }
