...
static struct mbuf *
_mbuf_get(struct mbuf_base *mbufb)
{
    struct mbuf *mbuf;
    uint8_t *buf;

    if (!STAILQ_EMPTY(&mbufb->free_mbufq)) {
        ASSERT(mbufb->nfree_mbufq > 0);

        mbuf = STAILQ_FIRST(&mbufb->free_mbufq);
        mbufb->nfree_mbufq--;
        STAILQ_REMOVE_HEAD(&mbufb->free_mbufq, next);

        ASSERT(mbuf->magic == MBUF_MAGIC);
        goto done;
    }

    buf = nc_alloc(mbufb->mbuf_chunk_size);
    if (buf == NULL) {
        return NULL;
    }
     ...
     *
     */
    mbuf = (struct mbuf *)(buf + mbufb->mbuf_offset);
...
struct mbuf *
mbuf_get(struct mbuf_base *mbufb)
{
    struct mbuf *mbuf;
    uint8_t *buf;

    mbuf = _mbuf_get(mbufb);
    if (mbuf == NULL) {
        return NULL;
    }

    buf = (uint8_t *)mbuf - mbufb->mbuf_offset;
    mbuf->start = buf;
    mbuf->end = buf + mbufb->mbuf_offset;

    ASSERT(mbuf->end - mbuf->start == (int)mbufb->mbuf_offset);
...
mbuf_put(struct mbuf *mbuf)
{
    ...
    mb->nfree_mbufq++;
    STAILQ_INSERT_HEAD(&mb->free_mbufq, mbuf, next);
}
 ...
 */
size_t
mbuf_data_size(struct mbuf_base *mbufb)
{
    return mbufb->mbuf_offset;
}
...
void
mbuf_init(size_t mbuf_chunk_size, struct mbuf_base *mbufb)
{
    mbufb->nfree_mbufq = 0;
    STAILQ_INIT(&mbufb->free_mbufq);

    mbufb->mbuf_chunk_size = mbuf_chunk_size;
    mbufb->mbuf_offset = mbufb->mbuf_chunk_size - MBUF_HSIZE;

    log_debug(LOG_DEBUG, "mbuf hsize %d chunk size %zu offset %zu length %zu",
              MBUF_HSIZE, mbufb->mbuf_chunk_size, mbufb->mbuf_offset, mbufb->mbuf_offset);
}

void
mbuf_deinit(struct mbuf_base *mbufb)
{
    while (!STAILQ_EMPTY(&mbufb->free_mbufq)) {
        struct mbuf *mbuf = STAILQ_FIRST(&mbufb->free_mbufq);
        mbuf_remove(&mbufb->free_mbufq, mbuf);
        mbuf_free(mbuf);
        mbufb->nfree_mbufq--;
    }
    ASSERT(mbufb->nfree_mbufq == 0);
