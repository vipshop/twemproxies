...
struct msg {
    ...
    unsigned             source_type:2;   /* proxy, redis or memcached? */

    struct msg_base      *mb;
};

struct msg *msg_tmo_min(struct msg_base *msgb);
void msg_tmo_insert(struct msg *msg, struct conn *conn);
void msg_tmo_delete(struct msg *msg);

void msg_init(struct msg_base *msgb, struct mbuf_base *mbufb);
void msg_deinit(struct msg_base *msgb);
struct string *msg_type_string(msg_type_t type);
struct msg *msg_get(struct conn *conn, bool request, unsigned source_type, struct msg_base *msgb);
void msg_put(struct msg *msg);
struct msg *msg_get_error(unsigned source_type, err_t err, struct msg_base *msgb);
void msg_dump(struct msg *msg, int level);
bool msg_empty(struct msg *msg);
rstatus_t msg_recv(struct context *ctx, struct conn *conn);
rstatus_t msg_send(struct context *ctx, struct conn *conn);
uint64_t msg_gen_frag_id(struct msg_base *msgb);
...
struct msg *req_get(struct conn *conn, struct msg_base *msgb);
...
struct msg *rsp_get(struct conn *conn, struct msg_base *msgb);
...
int nc_memory_init(struct context *ctx, size_t mbuf_chunk_size);
void nc_memory_deinit(struct context *ctx);
