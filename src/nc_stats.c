...
stats_add_header(struct stats *st)
{
    ...
    status = stats_add_num(st, &st->ncurr_conn_str, (int64_t)conn_ncurr_conn());
    if (status != NC_OK) {
        return status;
    }
...
struct stats *
stats_create(struct context *ctx, char *source)
{
    rstatus_t status;
    struct stats *st;
    ...
    string_set_text(&st->service, "nutcrackers");
...
static void
stats_thread_aggregate(struct stats *st, struct thread_data *tdata)
{
    uint32_t i;
    ...
    for (i = 0; i < array_n(&tdata->ctx->stats->current); i++) {
        ...
        stp1 = array_get(&tdata->ctx->stats->current, i);
        stp2 = array_get(&st->sum, i);
        stats_aggregate_metric(&stp2->metric, &stp1->metric);
